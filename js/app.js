(() => {
    ...
    function initSliders() {
        if (document.querySelector(".accessories__slider")) new core(".accessories__slider", {
            modules: [ Navigation, Autoplay, EffectFade, Lazy, Manipulation ],
            observer: true,
            observeParents: true,
            slidesPerView: 4,
            spaceBetween: 20,
    ...
    da.init();
    document.querySelector(".bicycle__total");
    const cart = document.querySelectorAll(".bicycle__add-to-cart");
    cart.forEach((cartItem => {
        cartItem.addEventListener("click", action);
        function action(e) {
            if (e.target.closest(".bicycle__total")) cartItem.parentElement.classList.toggle("_active");
        }
    }));
    const cartOpen = document.querySelector("._icon-cart");
    const page = document.querySelector(".page");
    document.querySelector(".header");
    document.querySelector(".accessories");
    const bodyElement = document.documentElement;
    document.addEventListener("click", openCart);
    function openCart(e) {
        if (e.target.closest(".header-store__cart")) {
            cartOpen.classList.toggle("open-cart");
            page.classList.toggle("cart-open");
            page.classList.remove("fullOpen");
            e.preventDefault();
        }
        if (e.target.closest("._icon-left")) {
            page.classList.toggle("cart-open");
            cartOpen.classList.remove("open-cart");
            e.preventDefault();
        }
    }
    window.addEventListener("load", pageLoaded);
    function pageLoaded(e) {
        bodyElement.closest(".menu__body");
        cartOpen.style.cssText = `\n  display: block;\n  position: relative;\n  z-index: 2;\n`;
    }
    window["FLS"] = true;
    isWebp();
    addTouchClass();
